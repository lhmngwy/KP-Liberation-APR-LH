			...
			if (_med isEqualTo 'AIS') then {
				...
				if (_med isEqualTo 'AWS') then {
					...
					if (_med isEqualTo 'ACE') then {
						...
						if (_med isEqualTo 'PAR') then {
							if (!isNil {_u getVariable 'PAR_isUnconscious'}) then {
								if ((_u getVariable 'PAR_isUnconscious') isEqualTo 1) then {
									_r = TRUE;
								};
							};
						};
...
_QS_fnc_iconColor = {
	...
	if (!(_v isKindOf 'Man')) then {
		if (_v getVariable ['QS_ST_drawEmptyVehicle',FALSE] || _x getVariable ["KPLIB_preplaced", false] || typeof _x == KPLIB_b_potato01) then {
...
_QS_fnc_iconUnits = {
	...
	if (((_di isEqualTo 1) && ((_QS_ST_X select 65))) && {(!(_QS_ST_X select 75))}) then {
		...
		if ((_QS_ST_X select 80)) then {
			{
				if (!(_x in _au)) then {
					if (_x getVariable ['QS_ST_drawEmptyVehicle',FALSE] || _x getVariable ["KPLIB_preplaced", false] || typeof _x == KPLIB_b_potato01) then {
