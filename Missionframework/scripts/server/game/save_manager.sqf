...
if (!isNil "_saveData") then {
    ...
    private _object = objNull;
    {
        ...
        if ((toLowerANSI _class) in KPLIB_classnamesToSave) then {
            ...
            if (!isNil "_fuelCargo" && !KPLIB_ace_refuel) then { _object setFuelCargo _fuelCargo; };
            if (!isNil "_fuelCargo" && KPLIB_ace_refuel) then { [_object, _fuelCargo] call ace_refuel_fnc_setFuel; };
